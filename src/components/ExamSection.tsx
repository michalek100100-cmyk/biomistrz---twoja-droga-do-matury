...
const ExamSection: React.FC<ExamSectionProps> = ({ onExamFinish }) => {
  ...
  const handleRestartExam = (e: React.MouseEvent, examId: string, file: string) => {
    e.stopPropagation();
    ...
    return (
      <div className="max-w-6xl mx-auto py-10 px-4">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-black text-gray-800 dark:text-white mb-4">Wybierz Arkusz</h2>
          <p className="text-gray-500 dark:text-gray-400 max-w-lg mx-auto">
            <code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded text-sm font-mono">Arkusze Maturalne Będą dodawane stopniowo</code>.
          </p>
        </div>

        {error && (
          <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-4 rounded-xl mb-8 flex items-center gap-3 border border-red-100 dark:border-red-900">
              ...
              <div 
                ...
                className={`group relative overflow-hidden rounded-3xl border-2 p-6 cursor-pointer transition-all hover:shadow-xl hover:-translate-y-1
                  ${hasProgress 
                    ...
                    : 'bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800 hover:border-purple-300 dark:hover:border-purple-700'
                  }`}
              >
                ...
                <div className="flex items-start justify-between mb-6">
                  ...
                  {hasProgress ? (
                    <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 text-xs font-black uppercase tracking-wide rounded-full animate-pulse">
                      W trakcie
                    </span>
                  ) : (
                    <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-black uppercase tracking-wide rounded-full">
                      {exam.level}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 text-xs font-bold text-gray-400 dark:text-gray-500 border-t border-gray-100 dark:border-gray-800 pt-4">
                ...
                <div className="mt-4 flex items-center justify-between">
                   ...
                   {hasProgress && (
                     <button 
                       onClick={(e) => handleRestartExam(e, exam.id, exam.file)}
                       className="p-2 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-400 rounded-full transition-colors"
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 pb-40 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="sticky top-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800 pt-[calc(env(safe-area-inset-top)+1rem)] pb-4 mb-8 -mx-4 px-4 md:-mx-8 md:px-8 shadow-sm transition-all flex justify-between items-center">
        <button 
          onClick={handleSaveAndExit}
          className="flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 font-bold transition-colors bg-white dark:bg-gray-800 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm"
        >
      ...
      <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 border border-gray-200 dark:border-gray-800 shadow-sm relative overflow-hidden mb-8">
          ...
          <div className="flex justify-between items-start mb-4">
            <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {currentExam?.tags?.map((tag, i) => (
              <span key={i} className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full text-xs font-bold border border-gray-200 dark:border-gray-700">
                #{tag}
              </span>
            ))}
          </div>
      ...
      <div className="space-y-8">
        {currentExam?.blocks.map((block) => (
          <div key={block.id} className="bg-white dark:bg-gray-900 p-6 md:p-8 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm">
              {renderBlockContent(
                block, 
                userAnswers, 
                isChecked, 
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-t border-gray-200 dark:border-gray-800 p-4 z-50">
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            
            {isChecked && (
              <div className="flex items-center gap-4 bg-gray-100 dark:bg-gray-800 px-6 py-3 rounded-2xl">
                 <div className="flex flex-col">
                    <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Twój Wynik</span>
                    <div className="flex items-baseline gap-1">
                       <span className="text-2xl font-black text-purple-600 dark:text-purple-400">{currentTotalScore}</span>
                       <span className="text-sm font-bold text-gray-400">/ {maxPossiblePoints} pkt</span>
                    </div>
                 </div>
                 <div className="h-8 w-[1px] bg-gray-300 dark:bg-gray-700 mx-2"></div>
            ...
            <div className="w-full md:w-auto">
              {!isChecked ? (
                <button 
                  ...
                  className="w-full md:w-auto flex items-center justify-center gap-3 px-8 py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-black text-sm uppercase tracking-wide shadow-xl transform hover:-translate-y-1 transition-all"
                >
                  Sprawdź odpowiedzi <CheckCircle2 className="w-5 h-5 text-emerald-400 dark:text-emerald-600" />
                </button>
              ) : (
                <button 
                  onClick={finishExam}
...
const renderBlockContent = (
      ...
      <button
        ...
        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all border-2 shrink-0
          ${isAwarded 
            ...
            : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-400 hover:border-emerald-400 hover:text-emerald-500'
          }
        `}
      >
        {isAwarded ? (
  ...
  switch (block.type) {
    case 'text':
      return <div className="prose max-w-none text-gray-800 dark:text-gray-300 whitespace-pre-wrap leading-relaxed">{block.value}</div>;
      ...
      return block.value ? (
        <div className="flex justify-center my-6 group relative cursor-zoom-in" onClick={() => onImageClick(block.value!)}>
          <img 
            src={block.value} 
            alt="Zadanie" 
            className="max-h-[500px] rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm transition-opacity hover:opacity-95" 
          />
    ...
    case 'question':
      return (
        <div className="space-y-4">
            {block.label && <h4 className="font-black text-gray-800 dark:text-white text-lg border-l-4 border-emerald-500 pl-3">{block.label}</h4>}
            <div className="space-y-4">
              {block.questionRows?.map((row) => {
                ...
                return (
                  <div key={row.id} className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
                    <div className="flex flex-col gap-4">
                      <div className="flex gap-4">
                        <span className="font-black text-gray-400 select-none">{row.points}.</span>
                        <div className="flex-1 space-y-3">
                          <p className="font-medium text-gray-800 dark:text-gray-200">{row.text}</p>
                          <textarea 
                            value={userAnswer}
                            ...
                            className={`w-full p-3 rounded-lg border-2 outline-none transition-all font-medium resize-none
                              ${isChecked 
                                ? 'border-gray-300 bg-gray-100 text-gray-600 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300' 
                                : 'border-gray-200 dark:border-gray-700 focus:border-purple-400 bg-white dark:bg-gray-800 dark:text-white'
                              }`}
                            disabled={isChecked}
                          />
                        </div>
                      ...
                      {isChecked && (
                        <div className="mt-2 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-in fade-in">
                          <div className="flex-1">
                            <span className="text-[10px] font-black uppercase text-emerald-600 dark:text-emerald-400 mb-1 block">Przykładowa odpowiedź:</span>
                            <p className="text-sm font-bold text-gray-700 dark:text-gray-300 bg-emerald-50 dark:bg-emerald-900/20 p-2 rounded-lg border border-emerald-100 dark:border-emerald-900/30">
                              {row.answerKey}
                            </p>
                          </div>
    ...
    case 'true_false_table':
      return (
        <div className="space-y-2">
          {block.label && <h4 className="font-black text-blue-600 dark:text-blue-400 text-xs uppercase tracking-wide mb-2"><Table className="inline w-3 h-3 mr-1"/> {block.label}</h4>}
          
          {/* ZMIANA TUTAJ: Dodano overflow-x-auto, aby tabela była przewijalna na małych ekranach */}
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-blue-100 dark:border-gray-700 overflow-x-auto">
            ...
            <div className="min-w-[500px] md:min-w-0"> 
              {block.tableRows?.map((row, idx) => {
                ...
                return (
                  <div key={row.id} className={`flex items-center justify-between p-3 ${idx % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-blue-50/30 dark:bg-gray-700/30'}`}>
                    <div className="flex-1 pr-4 min-w-[200px]">
                       <span className="font-medium text-sm text-gray-700 dark:text-gray-300">{row.statement}</span>
                       {isChecked && (
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                      <div className="flex gap-1">
                        {['P', 'F'].map((opt) => {
                          ...
                          let btnClass = "bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500";
    ...
    case 'grid_table':
      return (
        <div className="space-y-4">
          {block.label && <h4 className="font-black text-gray-800 dark:text-white text-sm uppercase tracking-wide text-center mb-2">{block.label}</h4>}
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 uppercase text-xs">
                <tr>
                  {block.gridHeaders?.map((h, i) => <th key={i} className="px-4 py-3 font-black text-center border-r border-gray-200 dark:border-gray-700 last:border-0">{h}</th>)}
                </tr>
              </thead>
                ...
                {block.gridRows?.map((row) => (
                  <tr key={row.id}>
                    {row.cells.map((cell) => {
                      ...
                      return (
                        <td key={cell.id} className="p-2 border-r border-gray-100 dark:border-gray-800 last:border-0 min-w-[120px]">
                          {cell.type === 'static' ? (
                            <span className="font-bold text-gray-700 dark:text-gray-300 pl-2">{cell.content}</span>
                          ) : (
                            <input 
                              type="text" 
                              value={userAnswers[cellUniqueId] || ''}
                              onChange={(e) => handleAnswerChange(cellUniqueId, e.target.value)}
                              className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded p-2 text-center focus:border-purple-400 outline-none dark:text-white"
                              disabled={isChecked}
                            />
                          )}
                        </td>
    ...
    case 'split_match_table':
      return (
        <div className="space-y-4">
          {block.label && <h4 className="font-black text-indigo-600 dark:text-indigo-400 text-xs uppercase tracking-wide mb-2"><Columns className="inline w-3 h-3 mr-1"/> {block.label}</h4>}
          <div className="bg-indigo-50/30 dark:bg-indigo-900/10 rounded-2xl p-6 border border-indigo-100 dark:border-indigo-800 overflow-x-auto">
             <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-6 items-center min-w-[500px] md:min-w-0">
                <div className="space-y-2">
                  {block.splitMatchLeft?.map((text, i) => (
                    <div key={i} className="flex gap-3 bg-white dark:bg-gray-800 p-3 rounded-xl border border-indigo-100 dark:border-indigo-900/50 shadow-sm">
                ...
                <div className="flex flex-col items-center gap-4 py-4 md:py-0">
                   ...
                   <div className="flex items-center gap-2 bg-white dark:bg-gray-800 p-2 rounded-xl border-2 border-indigo-100 dark:border-indigo-900/50 shadow-sm">
                      <input 
                        maxLength={1}
                        className="w-10 h-10 text-center font-black rounded-lg bg-indigo-50 dark:bg-indigo-900/30 border-transparent focus:bg-white focus:border-indigo-400 outline-none uppercase dark:text-white"
                        placeholder="A"
                      ...
                      <input 
                        maxLength={1}
                        className="w-10 h-10 text-center font-black rounded-lg bg-indigo-50 dark:bg-indigo-900/30 border-transparent focus:bg-white focus:border-indigo-400 outline-none dark:text-white"
                        placeholder="1"
                   ...
                   {isChecked && (
                     <div className="text-[10px] font-black bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 px-2 py-1 rounded">
